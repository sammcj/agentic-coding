...
description: Analyses skill usage patterns across conversations to track token consumption, identify heavy vs. lightweight skills, measure invocation frequency, detect co-occurrence patterns, and suggest consolidation opportunities. Use when the user asks to analyse skill performance, optimise skill usage, identify token-heavy skills, find consolidation opportunities, or review skill metrics.
---

# Skill Performance Profiler

Comprehensive analysis tool for tracking and optimising skill usage patterns, token consumption, and identifying opportunities for skill consolidation.
...
3. **Reporting**: Generate formatted output (markdown report, CSV, or visualisation)
...
Save this as `/Users/samm/.claude/conversations.json`.
...
```bash
cd /Users/samm/.claude
python3 /Users/samm/.claude/skills/skill-performance-profiler/scripts/analyze_skills.py conversations.json
```
...
- Skill categorisation (Lightweight/Medium/Heavy/Very Heavy)
...
python3 /Users/samm/.claude/skills/skill-performance-profiler/scripts/generate_report.py conversations_analysis.json markdown

# Generate CSV export
python3 /Users/samm/.claude/skills/skill-performance-profiler/scripts/generate_report.py conversations_analysis.json csv

# Generate both formats
python3 /Users/samm/.claude/skills/skill-performance-profiler/scripts/generate_report.py conversations_analysis.json both
```
...
2. **For detailed analysis**: Move the markdown report to `/Users/samm/.claude/skills-analyiser/outputs/` and provide a link
3. **For data exploration**: Create a spreadsheet (xlsx) or provide the CSV
4. **For visualisation**: Consider creating a React artefact with charts (using recharts library)

## Key Metrics Explained

**Invocation Count**: Number of times a skill was used across analysed conversations
...
→ Collect 20 recent chats, analyse, show top 5 heaviest skills
```
...
→ Analyse and group by time buckets
→ Create visualisation artefact with trend charts
```
...
For more accurate analysis, actual token counts from the API would be ideal, but this estimation provides useful relative comparisons for optimisation decisions.
...
- **Visualization artefact**: For trend analysis or comparative visualisations
- **Spreadsheet (xlsx)**: For detailed data exploration with built-in charts

Move all generated files to `/Users/samm/.claude/skills-analyser/outputs/` and provide computer:// links so users can download them.
