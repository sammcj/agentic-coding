...
Before running `hf upload-large-folder`, `hf upload`, or `hf sync` for a large transfer, run a pre-flight check. HF does not expose a public storage-quota API, but it does return IETF draft-9 `RateLimit` headers on every Hub API response (see https://huggingface.co/docs/hub/rate-limits).

1. Verify auth and account: `hf auth whoami --format json`
2. Check rate-limit headroom (5-minute fixed window, buckets: `api`, `pages`, `resolvers`):
   ```sh
   curl -sI -H "Authorization: Bearer $HF_TOKEN" https://huggingface.co/api/whoami-v2 | grep -i '^ratelimit'
   ```
   Header format: `RateLimit: "api";r=<remaining>;t=<seconds-to-reset>`. If `r` is low relative to the upload's expected request count, wait for `t` seconds before starting. 429 is returned on exceedance. Upload/commit-specific limits exist but are deliberately undocumented and change frequently.
3. Storage quota is not available via API. Direct the user to the billing dashboard for authoritative quota:
   - User: https://huggingface.co/settings/billing
   - Org: https://huggingface.co/organizations/<org>/billing
4. Estimate local payload: `du -sh <local_path>` and `find <local_path> -type f | wc -l`. Warn the user if size looks large relative to typical free-tier limits.
5. Prefer `upload-large-folder` (resumable, chunked) over `upload` for anything over a few GB or thousands of files.
