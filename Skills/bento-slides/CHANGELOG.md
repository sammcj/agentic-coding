...
- Compact authoring form is now the default; on-disk block still contains the full document
...
- `render_check.mjs`: rejects encrypted decks; hidden slides no longer overshoot the page walk
...
- `render_check.mjs`: boots with DNS off so shared decks cannot join their room (`--online` opts out); `--margin` flag
- `render_check.mjs --write`: keeps `<path>.bak`, copies input `collab` minus `sync` (a stale stamp restores deleted elements)
