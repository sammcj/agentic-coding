...
- Table: the header row is always weight 700 (`cell.bold` only affects body rows). `style.borderWidth` draws every edge of every cell; there is no per-edge control. Rules-only tables: `borderWidth:0` plus `rect` hairlines at the row pitch. A compact table with no `style` inherits `theme.table` defaults and dark-slide adjustments.
