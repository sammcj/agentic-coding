...
- For each source: apply the secret and PII filter, save it under `raw/` following the naming rules in `references/ingest.md`, then return a proposal. Do not compile.
...
6. Update `index.md` and `log.md` once, as the sole writer. Use a single batch bullet under today's date heading (OKF update-log form):
   ...
   * **Supersession**: [<old title>](<path>) superseded by [<new title>](<path>).
   ```
