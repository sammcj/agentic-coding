...
- `local/` - optional, gitignored. Your own personal notes (meeting prep, drafts, private sources), kept in this clone only and never committed. It may link into `wiki/` and `raw/`, but nothing committed links into it. See the llm-wiki skill's `references/local-content.md`.
- `SKILL.md` (project root) - optional. Lets an agent load this wiki as a query-only Agent Skill; maintenance still goes through the llm-wiki skill.
