...
def request_data(args):
    ...
    allowed = {"style", "tags", "lyrics", "cot", "seed", "abc", "abc_path", "cfg_scale", "id",
               "negative_style", "abc_sampling", "semantic_sampling"}
    if set(data) - allowed:
...
def run(args):
    ...
    sampling = {}
    if data is not None:
        # Sampling overrides are pipeline arguments, not SongRequest fields.
        sampling = {k: data.pop(k) for k in ("abc_sampling", "semantic_sampling") if k in data}
    ...
    write_json(output / "invocation.json", {
        ...
        "command": shlex.join([Path(sys.executable).name, *sys.argv]), "cwd": os.getcwd(),
        "requests": [r.to_dict() for r in requests], "sampling": sampling,
    })
    ...
    try:
        ...
        # Wall time spans model load and every request; result.json timing covers the pipeline call only.
        write_json(output / "run.json", {"results": results, "load_seconds": load_seconds,
...
def main():
    ...
    for action in ("generate", "all-modes", "plan", "decode"):
        ...
        command.add_argument("--device", default="auto", help="cuda, mps, cpu or auto (cuda, then mps, then cpu)")
        command.add_argument("--memory-budget-gib", type=float, default=24)
        command.add_argument("--quantization", choices=("none", "fp8", "int8", "auto"), default="auto")
